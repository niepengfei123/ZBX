...
public class CameraParamUtil {
    ...
    public Size getPropPreviewSize(List<Size> list, float th, int minHeight) {
        ...
        int i = 0;
        for (Size s : list) {
            if ((s.height >= minHeight) && equalRate2(s, th)) {
                Log.e(TAG, "PreviewSize:w = " + s.width + " h = " + s.height);
                break;
            }
            i++;
        }
    ...
    public Size getPropPictureSize(List<Size> list, float rate, int minHeight) {
        Collections.sort(list, sizeComparator);

        int i = 0;
        for (Size s : list) {
            if ((s.height >= minHeight) && equalRate(s, rate)) {
    ...
    public boolean equalRate2(Size s, float rate) {
        float r = (float) (s.width) / (float) (s.height);
